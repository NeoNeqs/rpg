...
public partial class StatCurves : Resource {
    ...
    static StatCurves() {
        ...
        Tools.Assert(DefaultReductionCurve!.ResourcePath.GetFile().Contains("reduction"), $"Possible resource mismatch. Expected a {nameof(Curve)} resource that defines default damage reduction.");
        Tools.Assert(DefaultPenetrationCurve!.ResourcePath.GetFile().Contains("penetration"), $"Possible resource mismatch. Expected a {nameof(Curve)} resource that defines default damage penetration.");
        Tools.Assert(DefaultStatCurve!.ResourcePath.GetFile().Contains("stat"), $"Possible resource mismatch. Expected a {nameof(Curve)} resource that defines default stat values.");
    }
#endif
}